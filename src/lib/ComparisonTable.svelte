	...
	<TableBody>
		{#each rowOptions as row}
			<TableBodyRow class="border-none">
				<TableBodyCell
					class="rounded-l-lg p-4 text-base font-normal whitespace-nowrap text-gray-500 dark:text-gray-300"
				>
					{row.name}
				</TableBodyCell>
				{#each columnOptions as column}
					...
					{:else}
						<TableBodyCell
							class="flex-shrink-0 text-right text-xs font-medium text-gray-900 sm:text-sm md:text-base dark:text-white"
						>
							{value}
						</TableBodyCell>
					{/if}
