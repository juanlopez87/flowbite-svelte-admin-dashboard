<script lang="ts">
	...
	let {
		...
		headingClass,
		spanClass,
	}: CategorySalesReportProps = $props();
	...
	const divCls = twMerge(
		'items-center justify-between border-b border-gray-200 pb-4 sm:flex dark:border-gray-700',
		divClass,
	);
	const headingCls = twMerge(
		'text-base font-normal text-gray-500 dark:text-gray-300',
		headingClass,
	);
	const spanCls = twMerge(
		'text-2xl leading-none font-bold text-gray-900 sm:text-3xl dark:text-white',
		spanClass,
	);
</script>
...
## Type
[CategorySalesReportProps](https://github.com/themesberg/flowbite-svelte-admin-dashboard/blob/main/src/lib/types.ts#L391)
## Props
@prop title
@prop titleTag = 'h3'
@prop subtitle
